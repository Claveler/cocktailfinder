...
async function downloadFile(url) {
  ...
  try {
    ...
    if (contentType.includes('text/html') || contentType.includes('text/plain')) {
      ...
      throw new Error(`Got HTML/text response instead of image data. Sample: ${sample}`);
    }
    
    // Check if content type is an image
    if (!contentType.startsWith('image/')) {
      console.warn(`⚠️ Unexpected content type: ${contentType}, but proceeding...`);
    }
...
async function migratePhotos() {
  console.log('🚀 Starting FIXED photo migration from Piscola2 to piscola-prod');
  console.log('====================================================');
