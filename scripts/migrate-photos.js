...
try {
  ...
  console.error(
    "📝 Please copy migration-config.example.js to migration-config.js and update with your credentials"
  );
  console.error(
    "💡 Run: cp scripts/migration-config.example.js scripts/migration-config.js"
  );
  process.exit(1);
}
...
async function migratePhotos() {
  ...
  try {
    ...
    if (!CONFIG.DEV.anonKey || !CONFIG.PROD.serviceRoleKey) {
      throw new Error(
        "❌ Please update migration-config.js with your actual Supabase credentials"
      );
    }
    ...
    if (!prodBuckets?.find((b) => b.id === "venue-photos")) {
      throw new Error(
        "❌ venue-photos bucket not found in production project. Run STEP1_CREATE_BUCKET.sql first!"
      );
    }
