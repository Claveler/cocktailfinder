        ...
        <div className="relative h-full p-4 flex flex-col justify-between text-white">
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-1.5 bg-black/60 backdrop-blur-sm px-2.5 py-1 rounded-full">
              ...
              <span className="text-xs text-white font-medium">
                {venue.pisco_status === "available"
                  ? "Pisco Available"
                  : venue.pisco_status === "unavailable"
                    ? "No Pisco"
                    : venue.pisco_status === "temporarily_out"
                      ? "Temporarily Out"
                      : "Status Unknown"}
              </span>
            </div>
          ...
          <div className="pt-3 border-t border-border">
            <div className="grid grid-cols-3 gap-4 text-xs mb-3">
              ...
              <div className="flex items-center justify-start gap-1.5 text-muted-foreground">
                ...
                <span className="font-medium">{venue.unique_verifiers}</span>
              </div>
  ...
  if (!onCardClick) {
    return (
      <Link href={`/venues/${venue.id}`} className="block">
        <Card className={getCardClasses()}>{cardContent}</Card>
      </Link>
    );
  }
