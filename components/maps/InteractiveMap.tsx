      ...
      >
        {venues
          ...
          .map((venue) => {
            ...
            const markerIcon = isClosest
              ? closestVenueMarkerIcon
              : venueMarkerIcon;
...
const InteractiveMap = memo(function InteractiveMap({
  ...
  const closestVenueId = useMemo(() => {
    ...
    const venuesWithLocation = approvedVenues.filter(
  ...
  const handleInternalBoundsChange = useCallback(
    (mapCenter: [number, number], mapZoom: number, bounds: MapBounds) => {
