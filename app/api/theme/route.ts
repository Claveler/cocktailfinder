...
export async function GET() {
  ...
  try {
    ...
    console.log('No active theme found, returning defaults. RPC Error:', rpcError, 'Table Error:', tableError);
    return NextResponse.json({
      colors: {
        primary: "#d32117",
        foreground: "#301718", 
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const requiredColors = ['primary', 'foreground', 'background', 'card', 'textAccent', 'secondary', 'accent', 'muted', 'border'];
    ...
    if (error) {
      ...
      if (error.message?.includes('relation "theme_settings" does not exist')) {
        return NextResponse.json(
          { error: "Theme system not initialized. Please run database migrations first." },
          { status: 503 }
        );
      }
